...
[role="lead"]
Vous pouvez déployer une instance d'évaluation de 90 jours d'un cluster ONTAP Select dont la taille varie d'un seul nœud à 99 nœuds en utilisant un modèle OVF sur VMware ESXi. Vous pouvez télécharger une licence d'évaluation si vous êtes un client existant ou un nouveau client. Par exemple, vous êtes déjà client et vous souhaitez essayer une nouvelle configuration avant d'acheter une licence.
...
* Vous devez installer le modèle OVF (contenu dans un fichier .ova) à l'aide du client autonome vSphere ou du client Web vSphere (seule option pour ESXi 8.0 et versions ultérieures). N'utilisez pas l'utilitaire d'administration ONTAP Select Deploy.
...
** Sélectionnez *Datacenter*. Ensuite, faites un clic droit et sélectionnez *Déployer le modèle OVF*.
