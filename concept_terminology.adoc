...
Cluster ONTAP Select:: Vous pouvez créer un cluster _ONTAP Select_ composé d'un, deux, quatre, six, huit, dix ou douze nœuds. Les clusters multi-nœuds contiennent toujours une ou plusieurs paires HA. Par exemple, un cluster à quatre nœuds est composé de deux paires HA. Un cluster à nœud unique ne fournit pas de fonctionnalité HA.
...
Pour plus d'informations, reportez-vous aux deux sections link:concept_workflow_deploy.html["Planification"] et link:concept_lic_evaluation.html["Licence"].
...
Magasin des identifiants:: Le magasin d'identifiants Deploy est une base de données sécurisée contenant les identifiants de compte. Il est principalement utilisé pour enregistrer des hôtes hyperviseur lors de la création d'un nouveau cluster. Reportez-vous à la section link:concept_workflow_deploy.html["Planification"] pour plus d'informations.
