...
Prise en charge de la gestion des clés externes (KMIP):: À partir de ONTAP Select 9.19.1, ONTAP Select prend en charge la gestion des clés externes à l'aide du protocole Key Management Interoperability Protocol (KMIP). Vous pouvez activer la gestion des clés externes après le déploiement d'un cluster ONTAP Select 9.19.1 ou la mise à niveau d'un cluster existant vers ONTAP Select 9.19.1. La gestion des clés externes est la seule gestion des clés prise en charge par ONTAP Select 9.19.1. Pour plus d'informations, consultez link:reference_lic_ontap_features.html#licensed-ontap-features["Fonctionnalités ONTAP concédées sous licence par défaut"].
+
--
ONTAP Select ne prend pas en charge les gestionnaires de clés cloud ni le gestionnaire de clés intégré ONTAP.

--
Prise en charge des processeurs Advanced Micro Devices (AMD):: À compter de ONTAP Select 9.19.1, ONTAP Select prend en charge les processeurs AMD sur les hôtes hyperviseur KVM (Kernel-Based Virtual Machine) et ESXi. Pour plus d'informations, consultez link:reference_plan_ots_hardware.html["Considérations relatives à l'hyperviseur VMware et au matériel"].
...
| ONTAP Select | Déploiement de ONTAP Select | link:task_install_deploy.html["Installez ONTAP Select Deploy"] 


| Installation de l'image ONTAP Select | Image ONTAP Select | link:task_cli_deploy_image_add.html["Ajoutez une image ONTAP Select à déployer"] 


| Mise à niveau de ONTAP Select Deploy | Déploiement de ONTAP Select | link:task_cli_upgrade_deploy.html["Mettez à niveau une instance de déploiement ONTAP Select"] 
...
--
Prise en charge de l'hyperviseur KVM mise à jour:: À partir de ONTAP Select 9.18.1, l'hyperviseur KVM est pris en charge sur Rocky Linux 10.1.
...
Prise en charge RAID logicielle pour les disques NVMe connectés localement sur les hôtes KVM:: ONTAP Select 9.17.1 introduit la prise en charge du RAID logiciel pour les disques NVMe connectés localement à ONTAP Select sur un hôte hyperviseur KVM.
+
--
...
--
Prise en charge du SnapMirror cloud:: À partir de ONTAP Select 9.17.1, SnapMirror cloud est pris en charge avec ONTAP Select. SnapMirror cloud est une fonctionnalité ONTAP sous licence qui est activée par défaut lorsque vous déployez un cluster ONTAP Select 9.17.1 ou mettez à niveau un cluster ONTAP Select existant vers la version 9.17.1.
+
--
https://docs.netapp.com/us-en/ontap/concepts/snapmirror-cloud-backups-object-store-concept.html["Découvrez les sauvegardes cloud SnapMirror vers le stockage objet"^].
...
--
Prise en charge de l'hyperviseur KVM mise à jour:: À partir de ONTAP Select 9.17.1, le hyperviseur KVM est pris en charge sur Red Hat Enterprise Linux (RHEL) 10.1, 10.0, 9.7 et 9.6 ainsi que sur Rocky Linux 10.0, 9.7 et 9.6.
+
--
[NOTE]
====
...
Mise à jour de la prise en charge des fichiers de licence NetApp:: À compter d'ONTAP Select 9.16.1, la prise en charge des fichiers de licence NetApp (NLF) est mise à jour. Le nouveau format NLF inclut les licences pour les fonctionnalités ARP, ONTAP S3 et S3 SnapMirror. link:reference_lic_ontap_features.html#licensed-ontap-features["En savoir plus >>"].
+
--
...
Prise en charge de ONTAP Select Deploy mise à jour:: Si vous utilisez une version de ONTAP Select Deploy 9.14.1 inférieure à 9.14.1P2, vous devez effectuer la mise à niveau vers ONTAP Select Deploy 9.14.1P2 dès que possible. Pour plus d'informations, consultez la link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["Notes de mise à jour de ONTAP Select 9.14.1"^].
...
Prise en charge de ONTAP Select Deploy mise à jour:: À compter d'avril 2024, ONTAP Select Deploy 9.13.1 n'est plus disponible sur le NetApp Support Site. Si vous utilisez ONTAP Select Deploy 9.13.1, vous devez effectuer une mise à niveau vers ONTAP Select Deploy 9.14.1P2 dès que possible. Pour plus d'informations, consultez la link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["Notes de mise à jour de ONTAP Select 9.14.1"^].
...
+
NetApp recommande fortement que les clients planifient et exécutent une migration complète des données depuis ONTAP Select for KVM vers toute autre plateforme ONTAP, y compris ONTAP Select for ESXi. Pour plus d'informations, consultez la  https://mysupport.netapp.com/info/communications/ECMLP2877451.html["Avis de fin de disponibilité"^]
