...
L'extension de cluster:: ONTAP Select 9.13.1 introduit l'extension de cluster qui offre une protection haute disponibilité contre l'impact d'une interruption de nœud sur la capacité d'un cluster à transmettre des données. À partir de la version ONTAP Select 9.13.1, vous pouvez étendre un cluster à un seul nœud à un cluster à deux nœuds en configurant les deux nœuds comme une paire haute disponibilité. La procédure est automatisée et initiée à partir de ONTAP Select Deploy à l'aide de l'interface de ligne de commande ou de l'interface web. Pour effectuer cette procédure, vous devez répondre à certaines exigences matérielles et logicielles. link:concept_cluster_expansion.html["En savoir plus >>"].
