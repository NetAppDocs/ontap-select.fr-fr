...
NetApp recommande d'utiliser ESX 8.0 ou une version ultérieure et un disque NVMe pour la banque de données hébergeant les disques système. Cette configuration offre les meilleures performances pour la partition NVRAM .


NOTE: Lors de l'installation sur ESX 8.0 ou version ultérieure, ONTAP Select utilise le pilote vNVMEe, que le disque système réside sur un SSD ou sur un disque NVMe. Cela définit le niveau matériel de la machine virtuelle sur 13, ce qui est compatible avec ESX 8.0 et versions ultérieures.
