...
NOTE: Depuis la version ONTAP Select 9.10.1, l'hyperviseur KVM n'est plus pris en charge lors de la création d'un nouveau cluster. De plus, depuis la version 9.11.1, vous ne pouvez plus gérer les clusters ONTAP Select existants sur KVM. Les questions concernant KVM ne s'appliquent qu'aux versions antérieures qui prennent en charge KVM.
