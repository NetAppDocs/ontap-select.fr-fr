...
. Sélectionnez l'icône en forme de silhouette en haut à droite de la page, puis sélectionnez *Changer le mot de passe*.
. Saisissez le mot de passe actuel et le nouveau mot de passe lorsque vous y êtes invité, puis sélectionnez *Soumettre*.
...
. Sélectionnez *Serveurs de gestion* > *Ajouter vCenter*.
. Saisissez les informations suivantes et sélectionnez *Ajouter*.
+
[cols="30,70"]
|===
...
|===
. Vous pouvez également sélectionner image:icon_kebab.gif["Options"] après l'ajout du nouveau serveur de gestion, puis sélectionner l'une des options suivantes :
+
